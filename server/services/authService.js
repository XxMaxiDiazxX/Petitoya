...
const verificarDatosUnicos = (documento, correo_electronico, telefono) => {
  return new Promise((resolve, reject) => {
    db.query('SELECT id_cliente FROM clientes WHERE id_cliente = ?', [documento], (err, idResults) => {
      ...
      if (idResults.length > 0) return resolve({ error: 'El ID del cliente ya existe' });

      db.query('SELECT correo_electronico FROM clientes WHERE correo_electronico = ?', [correo_electronico], (err, correoResults) => {
        if (err) return reject(err);
        if (correoResults.length > 0) return resolve({ error: 'El correo electrónico ya existe' });

        db.query('SELECT telefono FROM clientes WHERE telefono = ?', [telefono], (err, telefonoResults) => {
          if (err) return reject(err);
          if (telefonoResults.length > 0) return resolve({ error: 'El teléfono ya existe' });
...
const registrarUsuario = async (documento, nombre, correo_electronico, telefono, contrasena, rol) => {
  try {
    const hashedPassword = await encriptarContrasena(contrasena);
    await db.query('CALL CrearUsuario(?, ?, ?, ?, ?, ?)', [documento, nombre, correo_electronico, telefono, hashedPassword, rol]);
  } catch (error) {
    throw error;
  }
};
...
const autenticarUsuario = (documento) => {
  return new Promise((resolve, reject) => {
    db.query('CALL AutenticarUsuario(?, @p_contrasena, @p_nombre, @p_id_rol)', [documento], (err) => {
      if (err) return reject(err);
      db.query('SELECT @p_contrasena AS contrasena, @p_nombre AS nombre, @p_id_rol AS id_rol', (err, result) => {
        if (err) return reject(err);
...
const solicitarRestablecimientoContrasena = (correo_electronico) => {
  return new Promise((resolve, reject) => {
    db.query('SELECT id_cliente, nombre FROM clientes WHERE correo_electronico = ?', [correo_electronico], (err, result) => {
      if (err) return reject(err);
      if (!result || result.length === 0) return resolve({ error: 'Correo electrónico no encontrado' });
...
const verificarCodigoYRestablecerContrasena = async (id_cliente, codigo_verificacion, nueva_contrasena) => {
